...
class MatchGame extends Component {
  ...
  getFilteredProjects = () => {
    ...
    const filteredList = thumbnailList.filter(
      eachprojectDetails => eachprojectDetails.category === category,
    )
    return filteredList
  }
  ...
  render() {
    const {score, time, category, thumbnailList, imgUrl, isTrue} = this.state
    const filteredData = this.getFilteredProjects()
    return (
      <>
        <NavBar sc={score} ti={time} />
                  ...
                  <li key={e.tabId}>
                    <button
                      ...
                      className={`tab-button ${
                        category === e.tabId ? 'highlight-text' : ''
                      }`}
                      onClick={() => this.clickTab(e.tabId)}
                    >
                      {e.displayText}
                    </button>
                  </li>
